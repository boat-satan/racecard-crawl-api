...
def ex_flag(st_raw): return 1 if (st_raw and str(st_raw).strip().startswith("F")) else 0
def safe_load(path): 
...
def flatten_entry_course(stats_entry_course: dict) -> dict:
    out = {}
    if not stats_entry_course: return out
    ...
    for k, v in (stats_entry_course.get("winKimariteSelf") or {}).items(): out[f"win_{k}"] = v
    for k, v in (stats_entry_course.get("loseKimarite") or {}).items():   out[f"lose_{k}"] = v
...
def expand_trifecta_rows(integ, odds):
    ...
    lane_map = {int(e["lane"]): lane_feats(e) for e in integ.get("entries", [])}
...
def load_model_and_features():
    ...
    if os.path.exists(FEATS_JSON):
        with open(FEATS_JSON, "r", encoding="utf-8") as f:
            feats = json.load(f)
        return obj, feats
    raise RuntimeError("モデルは読み込めたが feature_cols が見つからない。train 時に feature_cols を保存するか、models/trifecta_feature_cols.json を用意して。")
...
def main():
    ...
    ap.add_argument("--pid",  required=True)   # ★ pid に統一
