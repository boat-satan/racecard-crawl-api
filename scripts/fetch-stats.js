...
// 出走表を探すルート（date優先 → today フォールバック）
const PROGRAM_ROOTS = [
...
async function collectRacersFromPrograms(){
  ...
  for(const root of PROGRAM_ROOTS){
    ...
    if(set.size) break; // date側で見つかったらtodayは見ない
  }
  return [...set];
}
...
async function main(){
  ...
  if(!racers.length){
    console.log("No racers to fetch. (Set RACERS env or put programs)");
    return;
  }
