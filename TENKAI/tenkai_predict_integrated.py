...
def _load_model_pair(t_model_date: Optional[str], k_model_date: Optional[str], pid: Optional[str]):
    ...
    k_mod = None; k_feats = None; k_dir = None; k_date_used = None; k_classes = None
    try:
    ...
    return {
        ...
        "kimarite": {"date": k_date_used, "dir": k_dir, "model": k_mod, "features": k_feats, "classes": k_classes},
    }
...
def _predict_kimarite(df_feat: pd.DataFrame, model, feat_cols: List[str], classes: List[str]) -> pd.DataFrame:
    ...
    if hasattr(model, "predict_proba"):
        ...
        # scikit or lgbm の classes_ に基づく並び対応
        model_classes = None
        try:
            model_classes = list(getattr(model, "classes_", None))
        except Exception:
            model_classes = None

        # 推論結果 → 指定 classes 順に整形
        prob_df = pd.DataFrame(prob_mat, columns=model_classes if model_classes else classes)
        # 欠けたラベルは0で補完、余剰は無視
        for c in classes:
            if c not in prob_df.columns:
                prob_df[c] = 0.0
        prob_df = prob_df[classes]
        for c in classes:
            base[f"prob_{c}"] = prob_df[c].astype(float).values
    else:
        # 確率が出せないモデルは全部0
        for c in cols: base[c] = 0.0
