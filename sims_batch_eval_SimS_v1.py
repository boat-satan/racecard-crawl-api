...
def main():
    ...
    # ★ predict専用モード（保存先は ./predict に固定・毎回上書き）
    ap.add_argument("--predict-only", action="store_true",
                    help="確率→TOPN出目だけを出力（ROI集計はしない）")
    # predout は残すがデフォルトは ./predict に変更
    ...
    if args.predict_only:
        ...
        # 出力先を毎回クリーンに（上書きOK）
        pred_dir = args.predout or "./predict"
